...
parser.add_argument('--dataset', default='yoochoose1_64', help='diginetica/Nowplaying/Tmall/yoochoose1_64')
...
parser.add_argument('--var', type=float, default=0.12)
...
def main():
    ...
    train_data = Data(train_data, category, avg_len, opt, True)
    test_data = Data(test_data, category, avg_len, opt, True)

    model = trans_to_cuda(CombineGraph(opt, num_node+n_category-1, n_category, category))

    logdir = f'./log/{opt.dataset}'
    # writer = SummaryWriter(logdir + '-' + opt.version + '-' + str(opt.ratio) + '-' + str(opt.max_length) + '-' + str(opt.seq_len) + '-' + str(opt.var))
